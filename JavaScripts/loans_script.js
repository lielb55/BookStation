...
availabilityDropdown.addEventListener("change", function () {
    filterTable();
    searchByNameFunction(); // Add this line to trigger the search function
});
...
function filterTable() {
    ...
    input = document.getElementById("availabilityDropdown").value;
    ...
    for (i = 0; i < tr.length; i++) {  // run for each row
        ...
        if (td && endLoanColumn) {
            ...
            if (input === "Any" || (input === "Active" && loanEndDate === "") || (input === "Not active" && loanEndDate !== "")) {
                tr[i].style.display = "";
                endLoanColumn.style.display = (input === "Not active") ? "none" : ""; // Hide "End loan" column for "Not active" filter
                endLoanHeader.style.display = (input === "Not active") ? "none" : ""; // Hide "End loan" header for "Not active" filter
            } else {
    ...
    // Add this line to trigger the search function
    searchByNameFunction();
}

// Add a global variable to store the previous filter state
let previousFilter = document.getElementById("availabilityDropdown").value;
